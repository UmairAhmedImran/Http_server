...
const char *level_to_string(log_level_t level) {
  switch (level) {
  case LOG_INFO:
    return (ANSI_COLOR_GREEN "INFO" ANSI_COLOR_WHITE);
  case LOG_WARNING:
    return (ANSI_COLOR_YELLOW "WARNING" ANSI_COLOR_WHITE);
  case LOG_ERROR:
    return (ANSI_COLOR_RED "ERROR" ANSI_COLOR_WHITE);
  case LOG_DEBUG:
    return (ANSI_COLOR_BLUE "DEBUG" ANSI_COLOR_WHITE);
  default:
    return (ANSI_COLOR_CYAN "UNKNOWN" ANSI_COLOR_WHITE);
  }
}
...
void log_message(log_level_t level, const char *format, ...) {
  ...
  // Calculate maximum possible size: timestamp(64) + level(10) + escaped
  // message(2048) + JSON overhead(~50)
  char json_log[2200];
  snprintf(json_log, sizeof(json_log), "%s: [%s]: %s", timestamp,
           level_to_string(level), escaped_buffer);
...
void log_request(const char *method, const char *path, const char *backend,
                 int status_code) {
  char timestamp[64];
  time_t now;
  struct tm *timeinfo;
...
void log_error(const char *component, const char *error_type,
               const char *details) {
  char timestamp[64];
  time_t now;
  struct tm *timeinfo;
  ...
  snprintf(json_log, sizeof(json_log),
           "%s: [%s]: %s: "
           "%s: %s",
           timestamp, level_to_string(LOG_ERROR), component, error_type,
           escaped_details);
